...
export async function getPostDetail(publicId: string, userId?: number | null) {
  ...
  const [tagRows, likeRows, commentRows] = await Promise.all([
    ...
    db.select({ userId: likes.userId }).from(likes).where(eq(likes.postId, post.id)),
    db
      .select({
        id: comments.id,
        body: comments.body,
...
async function getProfileByUserId(userId: number, viewerId?: number | null) {
  ...
  const userPosts = await hydrateTimeline(userPostsBase, viewerId);
...
export async function getProfileById(id: number, viewerId?: number | null) {
  return getProfileByUserId(id, viewerId);
}

export async function getProfileByHandle(handle: string, viewerId?: number | null) {
  const user = (await db.select({ id: users.id }).from(users).where(eq(users.handle, handle)).limit(1))[0];
  if (!user) return null;
  return getProfileByUserId(user.id, viewerId);
}
